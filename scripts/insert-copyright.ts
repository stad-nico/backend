...
async function insertDisclaimer(filePath: string) {
  ...
  if (!content.includes("Copyright (c)")) {
    ...
    return await writeFile(
      filePath,
      getDisclaimer(currentYear, author) + content
    );
  }
  ...
  if (!matches) {
    throw new Error(
      `Disclaimer already exists but no author or year could be matched in file ${filePath}`
    );
  }
  ...
  console.log(
    `${filePath} updated disclaimer ${matchedYear} -> ${currentYear}`
  );

  return await writeFile(
    filePath,
    content.replace(oldDisclaimer, getDisclaimer(currentYear, matchedAuthor))
  );
}
