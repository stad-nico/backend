...
import { DirectoryContentDirectory } from 'src/features/cloud/directories/mapping/contents/get-directory-contents.response';
...
export class DirectoryRepository extends EntityRepository<Directory> {
	public async getTotalByUserId(userId: string): Promise<number> {
		return this.em.createQueryBuilder(Directory).where({ user: userId }).getCount();
	}
	...
	public async getContentsRecursive(directoryId: string): Promise<{ files: Array<File>; directories: Array<Directory> }> {
		const childDirectories = this.em
			.createQueryBuilder(Tree)
			.select('child')
			.where({ parent: { id: directoryId } });
